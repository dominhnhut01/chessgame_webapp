...
class ChessAIEngine {
  ...
  calcEvaluationScore(chessBoard: any[][]): number {
    ...
    for (let type of this.types) {
      if (type !== "k") {
        material +=
          this.pieceWeights.get(type)! *
          (piecesInfo.get(type + "w")!.length -
            piecesInfo.get(type + "b")!.length);
      }
      
      positionScore += this.calcPostionScore(type, piecesInfo);
    }

    return this.chess.turn() == 'b' ? material + positionScore : - material - positionScore;
  }
  ...
  alphabeta(alpha: number, beta: number, depth: number): number {
    if (depth === 0)
      return this.quiesce(alpha, beta);
    let bestScore = -9999;
  ...
  computerMakingMove(): void {
    let bestMove: string = this.findBestMove(3);
    this.chess.move(bestMove);
  }

  updatePlayerMove(playerMove: string): void {
    this.chess.move(playerMove);
  }
}

export {ChessAIEngine};