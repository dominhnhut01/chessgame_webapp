...
class ChessAIEngine extends ChessEngine {
  ...
  private calcNextEvaluationScore(nextMove: Move): number {
    ...
    if (nextMove.flags.includes("p")) {
      curScore += this.pieceWeights.get('q')!;
    }
    return curScore;
  }
  ...
  private minimaxWithAlphaBeta(
    ...
    if (this.chess.isCheckmate()) return [99999, prevMove!];
  ...
  computerMakingMove(): string[] {
    ...
    if (this.openingMoves.length > 0) {
      bestMove = this.makeMove(this.openingMoves.pop()!);
    } else {
    ...
    //Update score
    this.curEvaluationScore = -(
      -this.curEvaluationScore + this.calcNextEvaluationScore(bestMove)
    );
  ...
  playerUndo(): Move | null {
    ...
    if (moveUndo) {
      if (!this.endOpeningMoves) this.openingMoves.push(moveUndo.san);
      console.log("undo");
